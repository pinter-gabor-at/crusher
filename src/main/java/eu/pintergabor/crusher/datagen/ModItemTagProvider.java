...
public final class ModItemTagProvider extends ItemTagsProvider {
	...
	private void tagGravelSources() {
		...
		tag.addOptionalTag(Tags.Items.STONES);
		tag.addOptionalTag(Tags.Items.COBBLESTONES);
		tag.add(
	...
	private void tagNormalFruitFoods() {
		final TagAppender<Item> tag = tag(FOODS_NORMAL_FRUIT);
		tag.add(
			ItemIds.APPLE,
	...
	private void tagNormalVegetableFoods() {
		final TagAppender<Item> tag = tag(FOODS_NORMAL_VEGETABLE);
		tag.add(
